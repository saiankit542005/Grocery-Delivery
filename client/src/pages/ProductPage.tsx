...
const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    api.get(`/products/${id}`).then(({ data }) => {
      ...
      return api
        .get(`/products?category=${data.product.category}`)
        .then(({ data }) => {
    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <nav className="flex items-center gap-2 text-sm text-app-text-light mb-6">
          ...
          <span>/</span>
          <span className="text-app-green font-medium max-w-[200px] ">
            {product.name}
          </span>
        </nav>
        {/* Back Button */}
        <button
          onClick={() => navigate(-1)}
          className="mb-6 flex items-center gap-1.6 text-sm text-app-text-light hover:text-app-green transition-colors"
        >
        ...
        <div className="bg-white/50 rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            {/* left side - Image */}
            <div className="relative flex items-center p-8 md:p-12 min-h-[320px] md:min-h-[480px]">
              <img
                className="max-h-[360px] w-auto object-cover"
                src={product.image}
                alt={product.name}
              />
              {/* Bages */}
              <div className="absolute top-5 left-5 flex flex-wrap gap-1.5">
                ...
                {product.discount > 0 && (
                  <span className="px-2.5 py-1 text-xs font-semibold bg-app-orange text-white rounded-xl">
            ...
            <div className="p-6 md:p-10 flex flex-col justify-center">
              <span className="text-xs font-semibold text-app-text-light tracking-wider mb-2 capitalize">
                {categoryLabel}
              </span>
              ...
              {/* Description */}
              <p className="text-sm text-app-text-light leading-related mb-6">
                {product.description}
              </p>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center border border-app-border rounded-xl overflow-hidden">
                  ...
                  <span className="w-12 text-sm font-semibold text-center">
                    {displayQuantity}
                  </span>
                  <button
                    onClick={handlePlus}
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center py-3 font-semibold rounded-xl transition-colors gap-2 ${
                    inCart
        ...
        {relatedProducts.length > 0 && (
          <section className="mt-12 mb-44">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 lg:gap-8 mt-6">
              {relatedProducts.slice(0, 5).map((rp) => (
                <ProductsCard key={rp.id} product={rp} />
              ))}
            </div>
          </section>
        )}
