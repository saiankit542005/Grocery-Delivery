...
export const inngest = new Inngest({ id: "go" });
...
const checkLowStock = inngest.createFunction(
  {
    ...
    triggers: [{ event: "invenroty/stock.updated" }],
  },
  async ({ event, step }) => {
...
const sendMonthlyOffers = inngest.createFunction(
  ...
  async ({ step }) => {
    const { deals, users } = await step.run("fetch-deals-users", async () => {
      //Get top discounted product as featured deals
    ...
    //Send in batches of 10 to aoid overwhelming mail server
    const batchSize = 10;
...
const autoAssignRider = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("assign-rider", async () => {
      ...
      //Find an active roider not currently delivering
